...
public class VentanaPrincipal {
    ...
    private void initialize() {
     ...
     // === Combo de alumnos para inscripciï¿½n ===
    ...
    private void enableAll() {
        ...
        JPanel pnCentro = (JPanel) frame.getContentPane().getComponent(1); // asumiendo que estï¿½ en BorderLayout.CENTER
        activarComponentes(pnCentro);
    }

    // Mï¿½todo recursivo para activar botones y combos
    private void activarComponentes(Container container) {
        for (Component comp : container.getComponents()) {
            if (comp instanceof JButton || comp instanceof JComboBox) {
                ...
                activarComponentes((Container) comp); // recursiï¿½n para subpaneles
            }
        }
    }
